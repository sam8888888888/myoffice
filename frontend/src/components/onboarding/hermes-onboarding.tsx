...
export function HermesOnboarding() {
  ...
  // Wizard klien (whitelabel): 3 langkah sederhana
  const [wiz, setWiz] = useState(0)
  const [brandName, setBrandName] = useState('')
  const [brandColor, setBrandColor] = useState('#6366F1')
  const [wizAgents, setWizAgents] = useState<string[]>(['rena', 'farrah', 'nadine'])
  ...
  const finishWizard = useCallback(async () => {
    try {
      localStorage.setItem('myoffice-brand-name', brandName)
      await fetch('/api/office?resource=branding&action=update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: brandName || undefined, primary_color: brandColor }),
      })
    } catch {
      // gagal simpan branding tidak menghalangi masuk
    }
    complete()
  }, [brandName, brandColor, complete])
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {step === 'welcome' && wiz === 0 && (
            <div className="space-y-4 text-center">
              ...
              <p className="text-sm" style={mutedStyle}>
                Setup 3 langkah — beri nama bisnis Anda, pilih agen, dan mulai.
              </p>
              <input
                value={brandName}
              ...
              <div className="flex items-center justify-center gap-2 text-sm">
                <span style={mutedStyle}>Warna utama</span>
                <input
                  type="color"
                  value={brandColor}
              ...
              <button
                ...
                className="w-full rounded-xl bg-accent-500 py-3 text-sm font-semibold text-white transition-colors hover:bg-accent-600 disabled:opacity-40"
              >
                Lanjut →
              </button>
          ...
          {step === 'welcome' && wiz === 1 && (
            <div className="space-y-4">
              <div className="text-center">
                <div className="text-4xl">🤖</div>
                <h2 className="text-lg font-bold">Pilih Agen Utama</h2>
                <p className="text-sm" style={mutedStyle}>
                  Agen yang paling sering bekerja untuk tim Anda.
                </p>
              </div>
              <div className="space-y-2">
                {[
                  ['rena', 'Koordinasi & Operasional'],
                  ['farrah', 'Ops & Bisnis'],
                  ['nadine', 'Proyek & Riset'],
                  ['aaron', 'Audit & Keamanan'],
                  ['dinda', 'Development'],
                ].map(([id, role]) => {
                  const on = wizAgents.includes(id)
                  return (
                    <button
                      key={id}
                      onClick={() =>
                        setWizAgents((prev) =>
                          on ? prev.filter((a) => a !== id) : [...prev, id],
                        )
                      }
                      className="flex w-full items-center justify-between rounded-xl px-4 py-3 text-left"
                      style={on ? { ...cardStyle, borderColor: 'var(--theme-accent)' } : cardStyle}
                    >
                      <span className="text-sm font-medium">{id}</span>
                      <span className="text-xs" style={mutedStyle}>{role}</span>
          ...
          {step === 'welcome' && wiz === 2 && (
            <div className="space-y-4 text-center">
              <div className="text-4xl">🚀</div>
              <h2 className="text-xl font-bold">Siap!</h2>
              <div className="rounded-xl p-4 text-left text-sm" style={cardStyle}>
                <div className="flex justify-between"><span style={mutedStyle}>Bisnis</span><span className="font-medium">{brandName}</span></div>
                <div className="mt-1 flex justify-between"><span style={mutedStyle}>Agen aktif</span><span className="font-medium">{wizAgents.length} agen</span></div>
                <div className="mt-1 flex justify-between"><span style={mutedStyle}>Mode</span><span className="font-medium">Whitelabel</span></div>
              </div>
              <button
          ...
          {step === 'connect' && (
            <div className="space-y-4 text-center">
              ...
              <p className="text-sm" style={mutedStyle}>
                Start by verifying that MyOffice can reach your
            ...
            <div className="space-y-4">
              ...
              <p className="text-center text-xs" style={mutedStyle}>
                {canEditConfig
                  ...
                  : 'This backend manages provider settings outside MyOffice. Confirm the model you expect to use, then test chat.'}
              </p>
